...
- [x] T022 SC-004 regression done. `country_scoreboard.parquet` is gitignored, so instead of a git diff the Major basket was rebuilt two ways on the **same panel** — current code vs. a reconstructed pre-T026 variant (cap lines stripped) — and the 9 value columns compared for the 12 Major countries. Result: **50 value diffs, all explained by T026** (every changed cell's pre-fix vintage was future-dated, 2030/2031), **zero unexplained changes**; original value columns all still present (additive schema otherwise). SC-004 in spec.md amended to record that T026 is the one intended value change. Harness: scratchpad `validate_phase6.py`.
- [x] T023 Edge case validated (`validate_phase6.py`): an **all-stale** basket still renders (rows present, every figure daggered — not hidden); an **all-N/A** basket still renders (explicit N/A cells, zero blank cells); a **truly-empty** basket (no rows) is cleanly omitted without error while the other baskets still render. The render's rule is "render any basket with ≥1 row", which matches spec.md's Edge Case intent.
- [x] T024 [P] Updated `specs/001-macro-pipeline/spec.md` — Country Scoreboard description now says three baskets (Major · Emerging Markets · Developing & Frontier, 26 economies) with as-of/staleness/N-A, and the IMF/yfinance/FRED source rows note the EM/Developing coverage (IMF: 24 areas; equity/policy: majors only, EM/Developing render N/A).
- [x] T025 Final validation. The full pipeline (`01_ingest`→`04_render`→newsletter) has run green on GitHub Actions three times across this feature (runs 30500668322, 30508970692, 30539688543), each publishing verified output (2→3 basket tables, 0 blank cells, no future vintages). A fully-local `01_ingest` pass isn't runnable without FRED/OECD/yf keys+network; the live runs are the authoritative end-to-end pass, complemented by the local harnesses that exec the real transform→model→render source against live IMF data.
