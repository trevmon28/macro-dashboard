...
**Known limitation (see tasks.md T026):** `_is_stale` only tests whether an as-of date is too *old*. It does not guard against a *future*-dated observation — IMF WEO includes forecast years (~2031), and `_latest`/`_prev` can select one, which then renders as "fresh" because its as-of is ahead of `NOW`. The US3 live run also exposed decades-old vintages (e.g. China GDP as-of 1961) being selected when recent years are NaN. Both are pre-existing vintage-selection behaviors that this feature made *visible*; the fix (cap selected vintage at `NOW`, prefer most-recent past vintage) is tracked as Phase 6 task T026.
